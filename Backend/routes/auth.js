 ...
 * - avatar preset + upload management
 * - OTP-based password reset flow
 */

// Rate limit for OTP-sensitive endpoints to reduce abuse and brute-force attempts.
...
const storage = multer.diskStorage({
    ...
    filename: function(req, file, cb) {
        // Include user id + timestamp to avoid filename collisions.
...
function createEmailTransporter() {
    // SMTP configuration is environment-driven so local/dev/prod can differ safely.
 ...
 * @param {string} userName Full name (first name is used in greeting).
 * @param {string} otp Six-digit one-time code.
...
/**
 * Creates signed auth token for user sessions.
...
/**
 * Normalizes user object for API responses.
 * Uses model helper when available, with legacy fallback shape.
 * @param {any} user
 * @returns {{id: *, name: string, email: string, role: string, avatar: string, createdAt: Date}|*}
 */
...
/* ---- PRESET AVATARS LIST ----
   GET /api/auth/preset-avatars
   Public. Returns categorised list of preset avatar paths.
*/
const AVATAR_IMG = /\.(png|jpg|jpeg|gif|webp)$/i;
const _avatarDirCache = {};

function readAvatarDir(subdir) {
    if (_avatarDirCache[subdir]) return _avatarDirCache[subdir];
    try {
        _avatarDirCache[subdir] = fs.readdirSync(path.join(__dirname, '..', 'Data', 'Avatar', subdir))
            .filter(function(f) { return AVATAR_IMG.test(f); })
            .map(function(f) { return subdir + '/' + f; });
    } catch (e) { _avatarDirCache[subdir] = []; }
    return _avatarDirCache[subdir];
}

router.get('/preset-avatars', function(req, res) {
    res.set('Cache-Control', 'public, max-age=3600');
    res.json({
        success: true,
        male: readAvatarDir('Male'),
        female: readAvatarDir('Female')
    });
});

/* ---- REGISTER ----
   POST /api/auth/register
   Body: { name, email, password }   Returns: { success, token, user }
*/
router.post('/register', async function(req, res) {
    try {
        ...
        // Sanitize gender
        const gender = ['male', 'female'].includes(req.body.gender) ? req.body.gender : 'other';

        // Assign random preset avatar based on gender
        const avatarSubdir = gender === 'female' ? 'Female' : 'Male';
        const avatarFiles = readAvatarDir(avatarSubdir);
        const avatar = avatarFiles.length
            ? avatarFiles[Math.floor(Math.random() * avatarFiles.length)]
            : '';

        // Create new user
        const user = await User.create({ name: trimmedName, email: email.trim(), password, gender, avatar });
...
router.put('/me', authMiddleware, async function(req, res) {
    try {
        ...
        if (avatar !== undefined) {
            ...
            // Accept: legacy numeric "1"-"5", new "Male/X.png" / "Female/X.png", or custom "uploads/..."
            const validAvatar = /^[1-5]$/.test(avatar)
                || /^(Male|Female)\/[^/]+$/.test(avatar)
...
/* ---- VERIFY OTP ----
    ...
    Body: { email, otp }
    Validates code and opens a short reset window for password update.
*/
...
/* ---- RESET PASSWORD ----
    ...
    Requires prior OTP verification and a still-valid reset window.
*/
