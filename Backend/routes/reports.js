 ...
 *
 * Split access model:
 * - Submit endpoint is public (token optional for user linkage)
 * - Management endpoints require admin or tester role
 */

/**
 * POST /api/reports
 * Body: contextual issue metadata + required description.
 *
 * Creates a new report document; if bearer token is valid, associates `userId`.
 */
...
/**
 * GET /api/reports
 * Query: page, limit, status, category, language, exampleTitle, search, sort
 *
 * Returns filtered/paginated moderation list with reporter info when available.
 */
 ...
 *
 * Returns aggregated moderation stats grouped by status/category/example/language.
 */
